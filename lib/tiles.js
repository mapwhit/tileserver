...
function serveData(options, params, id) {
  const tileJSON = {
    tilejson: '2.0.0',
    name: id,
    format: 'pbf'
  };
  const domains = params.domains || options.domains;
  ...
  router.get('/:z/:x/:y.pbf', checkParams, getTile, zipTile, sendTile);
  ...
  function sendData(req, res) {
    const { format, ...data } = tileJSON;
    const info = {
      ...data,
      tiles: getTileUrls(req, domains, `data/${id}`, format, {
